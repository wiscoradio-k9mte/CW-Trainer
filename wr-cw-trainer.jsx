      ...
      {qso && !done && cur && cur.who === "dx" && (
        <div style={S.panel}>
          ...
          {difficulty === "easy" ? (
            ...
            <>
              {/* Real <label htmlFor>, not the parallel aria-label this used to carry:
                  the old accessible name ("Your copy of what you heard") did not contain
                  the visible caption, which is WCAG 2.5.3 failure F96 — a speech-input
                  user saying the full visible label hit nothing. display:"block" preserves
                  the geometry the caption had as a <div>; <label> is display:inline by
                  default, whose vertical margins don't apply. */}
              <label htmlFor={copyInputId} style={{ ...S.label, display: "block", marginBottom: 6 }}>Your copy (optional — check it or just answer)</label>
...
function Settings({ settings, setSettings, onClose }) {
  ...
  // Ids for the caption/gloss text that names the RX-filter group and the cut-numbers
  // toggle. useId rather than constants so uniqueness doesn't rest on the current
  // mount-exclusivity invariant (the two Settings render sites gate on
  // `showSettings && !isWide` and `isWide && showSettings`, so only one is ever live).
  const uid = useId();
